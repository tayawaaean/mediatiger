...
export const SecuritySection: React.FC = () => {
      ...
      try {
        ...
        if (factorId) {
          try {
            ...
            console.error(translate('failedToCleanupUnverifiedFactor'), cleanupErr);
          }
        }
  ...
  return (
      <section className="pt-2">
        <SectionTitle icon={<ShieldCheck size={16} />} title={translate('security')} />
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between bg-slate-700/30 p-3 rounded-lg border border-slate-700/50 transition-all duration-300 hover:bg-slate-700/40">
            <div>
              <h3 className="text-slate-200 font-medium">{translate('twoFactorAuthentication')}</h3>
              <p className="text-xs text-slate-400 mt-0.5">{translate('addExtraLayerOfSecurity')}</p>
            </div>
            <div>
              <Toggle
          ...
          {showTwoFactorSetup && (
              <div className="mt-4 p-4 bg-indigo-900/20 border border-indigo-800/30 rounded-lg animated-fade-in">
                <h4 className="font-medium text-indigo-300 mb-2">{translate('setupTwoFactorAuthentication')}</h4>
                <p className="text-sm text-slate-300 mb-4">
                  {translate('scanQRCodeWithAuthenticatorApp')}
                </p>
                <div className="flex justify-center p-6 bg-white rounded-lg mb-4">
                  <img src={qrCode} alt={translate('twoFAQRCode')} className="w-40 h-40 object-contain" />
                </div>
                <Input
                    type="text"
          ...
          {showTwoFactorDisable && (
              <div className="mt-4 p-4 bg-red-900/20 border border-red-800/30 rounded-lg animated-fade-in">
                <h4 className="font-medium text-red-300 mb-2">{translate('disableTwoFactorAuthentication')}</h4>
                        ...
                        try {
                          ...
                          for (const factor of factorData?.all || []) {
                            if (factor.status === 'unverified') {
                              console.log(`${translate('removingUnverifiedFactor')}: ${factor.id}`);
