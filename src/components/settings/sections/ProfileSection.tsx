...
import { User, PenSquare, Lock, UserCircle, Pencil } from "lucide-react";
...
export const ProfileSection: React.FC<ProfileSectionProps> = ({
  ...
  const { uploadingImage, profileImage, handleImageUpload } =
    useUserProfile(user);
  ...
  const handleSaveChanges = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      setIsEditing(false);
      toast.success(
        "Profile updated successfully. Please check your email to confirm changes if required."
      );
    } catch (error: any) {
  ...
  const handleImageUploadWithStopPropagation = (
    ...
    <section className="pt-2" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="mt-4 flex gap-4 items-start">
        <div className="flex-shrink-0">
          ...
          <div
            className="h-16 w-16 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xl font-bold relative overflow-hidden border-2 border-indigo-400/30 group "
            ...
            {profileImage ? (
              <img
                ...
                onError={(e) => {
                  ...
                  e.currentTarget.parentElement!.innerHTML = name
                    .charAt(0)
                    .toUpperCase();
                }}
              />
        ...
        <div className="flex-1 w-full">
          ...
          >
            <Pencil size={15} style={{ paddingRight: 2 }} />
            {safeTranslate("edit")}
          </Button>
          ...
          >
            <Lock size={15} style={{ paddingRight: 2 }} />
