        ...
        <div className="bg-slate-700/30 backdrop-blur-sm rounded-xl p-3 md:p-6 mb-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <option value="pending">{translate('youtubeChannels.pending')}</option>
                <option value="approved">{translate('youtubeChannels.approved')}</option>
                <option value="rejected">{translate('youtubeChannels.rejected')}</option>
              </select>
              <button
          ...
          {isLoadingApplications ? (
              <div className="text-center py-8">
                ...
                <p className="text-slate-300">{translate('youtubeChannels.loadingApplications')}</p>
              </div>
          ) : !channelsRequests || channelsRequests.length === 0 ? (
              <div className="text-center py-8 text-slate-400">
                <p>{translate('youtubeChannels.noApplicationsFound', { status: translate(`youtubeChannels.${applicationFilter}`) })}</p>
              </div>
          ) : (
              ...
              <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
                <div className="bg-slate-800 rounded-xl p-6 max-w-md w-full mx-4">
                  ...
                  <textarea
                      ...
                      placeholder={translate('youtubeChannels.enterReasonForRejection')}
                  />
  ...
  return (
      <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-4 md:p-6 border border-slate-700/50 hover:border-indigo-500/50 transition-all duration-300 group">
        <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-4">
          ...
          <div className="flex items-center gap-2">
            {app.status === "pending" && (
                <>
                  <button
                      onClick={() => handleApplicationStatus(app.id, "approved", translate('youtubeChannels.approved'))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
          <div>
            <h4 className="text-sm font-medium text-slate-400 mb-2">{translate('youtubeChannels.interests')}</h4>
        ...
        <div className="mt-6 pt-4 border-t border-slate-700/50 flex items-center justify-between text-xs text-slate-400">
          <span>{translate('youtubeChannels.submitted')}: {new Date(app.created_at).toLocaleString()}</span>
          <span
