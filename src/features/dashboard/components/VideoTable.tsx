...
const VideoTable: React.FC<VideoTableProps> = ({ data, isLoading = false }) => {
  ...
  const avgRpmGross = totals.views > 0 ? (totals.revenueGross / totals.views) * 1000 : 0;
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-slate-700/50">
              <th className="px-6 py-4 text-left text-xs font-semibold text-slate-400">
                ...
                >
                  VIDEO
                  <span className={isSorting && sortField === 'title' ? 'animate-spin' : ''}>
                    {getSortIcon('title')}
                  </span>
                </button>
              </th>
          ...
          <tbody className={isLoading ? 'opacity-50' : ''}>
            <tr className="border-b border-slate-700/50 bg-slate-800/30">
              ...
              <td className="px-6 py-4 text-left text-sm">{formatCurrency(avgRpmGross)}</td>
              <td className="px-6 py-4 text-left text-sm">{formatCurrency(totals.revenueGross)}</td>
              <td className="px-6 py-4 text-left text-sm">{formatNumber(totals.views)}</td>
            </tr>
            {isLoading ? (
              <>
                <LoadingRow />
              ...
              sortedData.map((video) => (
                <tr key={video.id} className="border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      {video.thumbnail && (
                  ...
                  <td className="px-6 py-4 text-left text-sm">{formatCurrency(video.rpmGross)}</td>
                  <td className="px-6 py-4 text-left text-sm">{formatCurrency(video.revenueGross)}</td>
                  <td className="px-6 py-4 text-left text-sm">{formatNumber(video.views)}</td>
                </tr>
              ))
            )}
          </tbody>
