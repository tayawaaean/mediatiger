...
const BenefitItem: React.FC<BenefitItemProps> = ({ icon, text, index }) => (
  <div
    className="group flex items-start space-x-3 sm:space-x-4 p-4 sm:p-6 md:p-8 lg:p-12 py-4 sm:py-5 md:py-6 w-fit rounded-xl sm:rounded-2xl bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 hover:border-indigo-500/50 transition-all duration-500 hover:bg-slate-700/40 hover:shadow-2xl hover:shadow-indigo-500/10 transform hover:scale-[1.02]"
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      <p className="text-slate-200 text-sm sm:text-base md:text-lg leading-relaxed font-medium group-hover:text-white transition-colors duration-300">
        {text}
      </p>
    </div>
...
const WhyCreatorsLove: React.FC = () => {
  ...
  return (
    <div
      ref={sectionRef}
      className={`relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 py-12 sm:py-16 md:py-20 lg:py-24 overflow-hidden ${sectionClasses}`}
    >
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10">
        ...
        <div
          ref={headerRef}
          className={`text-center mb-8 sm:mb-12 md:mb-16 ${headerClasses}`}
        >
          <h2 className="relative inline-block mb-4 sm:mb-6 ">
            <span className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl gap-2 sm:gap-4 font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 via-purple-200 to-indigo-300 tracking-tight leading-tight drop-shadow-2xl">
        ...
        <div
          ref={ctaRef}
          className={`text-center mt-8 sm:mt-12 md:mt-16 ${ctaClasses}`}
        >
          <div className="inline-flex items-center px-4 sm:px-6 md:px-8 py-3 sm:py-4 bg-gradient-to-r from-slate-800/80 to-slate-700/80 backdrop-blur-sm border border-slate-600/50 rounded-full shadow-2xl hover:shadow-indigo-500/20 transition-all duration-300 group">
            <span className="text-slate-300 font-medium text-sm sm:text-base md:text-lg mr-2 sm:mr-3 group-hover:text-white transition-colors">
